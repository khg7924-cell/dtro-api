...
def export_treeview_to_excel(tree, filename_prefix):
    try:
        ...
        if not data:
            messagebox.showwarning("저장 오류", "저장할 데이터가 없습니다. 먼저 조회해 주세요.")
            return
        ...
        if file_path:
            df = pd.DataFrame(data, columns=display_headers)
            df.to_excel(file_path, index=False)
            messagebox.showinfo("저장 완료", f"데이터가 성공적으로 저장되었습니다.\n경로: {file_path}")
    except Exception as e:
        messagebox.showerror("저장 실패", f"엑셀 저장 중 오류가 발생했습니다: {e}")
...
if login_success:
    ...
    # ==========================================
    # 🌟 [탭 2] 전력량 & 기상 통합 모니터링 (데모용 시뮬레이션 완벽 탑재!)
    # ==========================================
    ...
    def load_elec_and_weather_data():
        ...
        # 🎯 API 값이 없으면 시뮬레이션용 가짜(Dummy) 날씨 데이터 강제 생성 (항상 꽉 찬 화면 유지!)
        if h_data:
            weather_dict = {d['date']: d for d in h_data}
        else:
            messagebox.showinfo("시뮬레이션 모드", "한전 API 승인 전이므로, 시연을 위한 가상의 전력량 및 기상 데이터를 생성하여 화면을 채웁니다.")
            for d_str in date_list:
                fake_w = {
                    'code': random.choice([0, 1, 2, 3, 61]),
                    'max': random.uniform(22.0, 35.0),
                    'min': random.uniform(15.0, 24.0),
                    'hum': random.uniform(40, 85),
                    'pm10': random.uniform(20, 60),
                    'pm25': random.uniform(10, 35)
                }
                weather_dict[d_str] = fake_w
                h_data.append({'date': d_str, **fake_w})
        ...
        for day_str in date_list:
            ...
            w_desc = WMO_MAP.get(w_info.get('code', 0), "맑음")
            w_max = safe_format(w_info.get('max', 25.0))
            w_min = safe_format(w_info.get('min', 15.0))
            w_hum = safe_format(w_info.get('hum', 50), is_int=True)
            w_pm10 = safe_format(w_info.get('pm10', 30), is_int=True)
            w_pm25 = safe_format(w_info.get('pm25', 15), is_int=True)
