...
# =========================================================================
# 🚀 2. 연도별 비교 분석 (계절별 기온 분리 및 전사적 절전 분석)
# =========================================================================
...
def get_compare_data(station: str, base_year: str, comp_year: str, price: int = 150):
    ...
    # 🌟 [핵심] 부하가 증가하는 조건: (여름이 더워짐 OR 겨울이 추워짐) OR 승객 증가
...
def get_predict_data(station: str, target_year: str, pass_rate: float = 0.0, temp_adj: float = 0.0):
    ...
    if df['temp_max'].isna().all() or len(asos_data) == 0:
        return {"error": "기상청 공공데이터포털 서버 응답이 없습니다. 팩트 기반 분석을 위해 가상 데이터를 사용하지 않습니다."}
    ...
    # 🌟 예측 기준: '직전 연도'만 가져오도록 명확히 필터링 (23~25년 통째로 합쳐지는 버그 해결)
    ...
    X_train = train_df[features].drop(columns=['date']).fillna(method='bfill').fillna(method='ffill')
    y_train = train_df['target_power']
    X_test = test_df[features].drop(columns=['date']).fillna(method='bfill').fillna(method='ffill')
    
    model = RandomForestRegressor(n_estimators=100, random_state=42)
    model.fit(X_train, y_train)
    test_df['pred_power'] = model.predict(X_test)
    
    # 🌟 직전 1개 연도의 전력량 합계 산출 (비교 기준 완벽 정상화)
