...
import requests
import xgboost as xgb  # 🚀 지시하신 XGBoost 엔진 탑재
...
def get_predict_data(station: str, target_year: str, pass_rate: float = 0.0, temp_adj: float = 0.0):
    try: # 🚨 예측 실패나 파이썬 에러 시 화면에 알림을 띄우기 위한 무적 방어막
        ...
        # 🚨 500 통신 에러 핵심 원인 차단: 
        # Numpy.float 타입을 FastAPI가 JSON으로 못 바꿔서 서버가 뻗는 현상을 float()를 씌워 완벽 방어
        ...
        return {
            "summary": { 
                "last_tot": lt, 
                "tot_future": ft, 
                "last_peak": last_peak_val, 
                "peak_future": peak_future_val, 
                "acc": r2_acc 
            }, 
            "chart_data": records, 
            "feat_data": top_feats
        ...
        # 🚨 서버가 조용히 죽지 않고 무조건 원인을 프론트엔드로 쏴주도록 조치
        return {"error": f"AI 분석 서버 연산 오류: {str(e)}\n\n상세로그:\n{traceback.format_exc()}"}