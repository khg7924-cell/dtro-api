...
def get_dashboard_data(station: str, start: str, end: str):
    ...
    for i in range(diff):
        ...
        records.append({
            ...
            "co2": round(day_co2, 2), "pfLag": 98.5, "pfLead": 99.0,
            "temp_max": kma.get("tmax", "--"), "temp_min": kma.get("tmin", "--"), "humidity": kma.get("humi", "--"),
            "pm25": "--", "details": details 
        })
...
# =========================================================================
# 🚀 3. AI 수요 예측 (과거 다년간의 데이터는 업로드된 엑셀을 기반으로 XGBoost 학습 수행)
# =========================================================================
