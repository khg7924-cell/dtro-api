...
# 🌟 [에러 원인 1 완벽 해결] 배포 환경의 치명적 CORS 통신 에러 수정
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_credentials=False, # True일 경우 배포 브라우저에서 무조건 통신을 차단함 (False로 수정 완료)
    allow_methods=["*"],
    allow_headers=["*"],
)
...
def get_compare_data(station: str, base_year: str, comp_year: str, price: int = 150):
    try:
        ...
        # 🌟 파이썬 에러 시 뻗지 않고 프론트엔드에 상세 원인을 알림
        return {"error": f"비교 분석 중 서버 에러가 발생했습니다: {str(e)}\n{traceback.format_exc()}"}

# =========================================================================
# 🚀 3. AI 수요 예측 (에러 방어 완벽 조치 및 XGBoost 학습)
# =========================================================================
...
def get_predict_data(station: str, target_year: str, pass_rate: float = 0.0, temp_adj: float = 0.0):
    try:
        ...
        # 🌟 공공데이터포털 서버 장애 시 단호하게 분석 거부 (Fail-Safe 제거)
        if df['temp_max'].isna().all() or len(asos_data) == 0:
            return {"error": "기상청 공공데이터포털 서버와 통신할 수 없습니다. 팩트 기반 정밀 분석을 위해 가상 기후를 삽입하지 않습니다. 공공데이터포털 복구 후 다시 시도해 주세요."}
        
        # XGBoost는 NaN(결측치)을 스스로 무시하고 학습할 수 있으므로, 억지 결측치 보간(ffill) 코드를 전부 삭제!
        ...
        # 타겟 전력량(정답지)이 없는 과거 데이터는 모델 학습이 불가능하므로 제거
        train_df = train_df.dropna(subset=['target_power'])
        if train_df.empty: return {"error": "학습에 필요한 '전력량 실측치' 데이터가 존재하지 않아 분석이 불가능합니다."}
             
        test_df = df[df['date'].dt.year == target_y].copy()
        if test_df.empty: return {"error": f"{target_year}년도 예측을 위한 엑셀 포맷(빈 날짜 행)이 없습니다."}
        
        X_train = train_df[features].copy()
        y_train = train_df['target_power'].copy()
        ...
        name_map = {'temp_max': '기온', 'humidity': '습도', 'passengers': '이용 승객수', 'pm25': '초미세먼지', 'pm10': '미세먼지', 'is_offday': '휴일 여부'}
        ...
        # 🌟 배포 서버 크래시 튕김 현상(Failed to Fetch)을 막는 궁극의 방어 로직
        return {"error": f"AI 분석 중 엑셀 데이터 양식 오류 또는 서버 내부 에러가 발생했습니다:\n\n{str(e)}\n\n{traceback.format_exc()}"}