...
FIREBASE_URL = "https://datacenter-app-7a69a-default-rtdb.firebaseio.com/users"
...
COLOR_COMPARE = "#8B5CF6"
COLOR_PREDICT = "#EC4899"
COLOR_ACTUAL = "#00A651"
...
# 🎯 누락되었던 폰트 시스템 완벽 탑재
FONT_FAMILY = "Malgun Gothic" if os.name == 'nt' else "AppleGothic"
FONT_NUM = "Segoe UI"
FONT_XL = (FONT_NUM, 42, "bold")
FONT_L = (FONT_NUM, 22, "bold")
...
weather_canvas = None; weather_annot = None; weather_highlight_points = []

elec_chart_data = []
elec_fig = Figure(figsize=(4, 2.5), dpi=100)
elec_canvas = None; elec_ax1 = None; elec_ax2 = None; elec_annot = None; elec_highlight_points = []
...
def safe_format(val, suffix="", is_int=False):
    ...
    try: return f"{int(float(val))}{suffix}" if is_int else f"{float(val):.1f}{suffix}"
    except: return "--" + suffix

def get_pm_status(val, dust_type="pm25"):
    try:
        v = float(val)
        if dust_type == "pm10": return ("좋음",COLOR_PRIMARY) if v<=30 else ("보통",COLOR_SECONDARY) if v<=80 else ("나쁨","#F59E0B") if v<=150 else ("매우 나쁨","#EF4444")
        else: return ("좋음",COLOR_PRIMARY) if v<=15 else ("보통",COLOR_SECONDARY) if v<=35 else ("나쁨","#F59E0B") if v<=75 else ("매우 나쁨","#EF4444")
    except: return "정보 없음", COLOR_TEXT_SUB

WMO_MAP = {0:"☀️ 맑음", 1:"🌤 맑음", 2:"⛅ 구름", 3:"☁️ 흐림", 45:"🌫 안개", 48:"🌫 안개", 51:"🌦 이슬비", 53:"🌦 이슬비", 55:"🌦 강비", 61:"🌧 비", 63:"🌧 비", 65:"🌧 강비", 71:"🌨 눈", 73:"🌨 눈", 75:"🌨 강눈", 80:"🌦 소나기", 81:"🌧 소나기", 82:"🌧 강소나기", 95:"⛈ 뇌우", 96:"⛈ 뇌우", 99:"⛈ 강뇌우"}
...
def get_weather_desc_from_code(sky, pty):
    if pty in ['1', '4', '5']: return "🌧 비"
    elif pty in ['2', '6']: return "🌨 비/눈"
    elif pty in ['3', '7']: return "❄️ 눈"
    else: return "☀️ 맑음" if sky == '1' else "⛅ 구름많음" if sky == '3' else "☁️ 흐림" if sky == '4' else "🌤 맑음"
...
def get_current_hybrid_weather(lat, lon):
    ...
    try:
        ...
        w["status"] = get_weather_desc_from_code(w["sky"], w["pty"])
        return w
    except: return None

def get_forecast_weather(lat, lon):
    try:
        url = f"https://api.open-meteo.com/v1/forecast?latitude={lat}&longitude={lon}&hourly=temperature_2m,weathercode&daily=weathercode,temperature_2m_max,temperature_2m_min&timezone=Asia%2FSeoul"
        res = requests.get(url, timeout=5).json()
        daily = res.get('daily', {}); forecast_daily = []
        for i in range(len(daily.get('time', []))):
            forecast_daily.append({'date': daily['time'][i], 'code': daily['weathercode'][i] if i < len(daily.get('weathercode', [])) else 0, 'max': daily['temperature_2m_max'][i] if i < len(daily.get('temperature_2m_max', [])) else 0, 'min': daily['temperature_2m_min'][i] if i < len(daily.get('temperature_2m_min', [])) else 0})
            
        hourly = res.get('hourly', {}); forecast_hourly = []; now = datetime.now()
        if 'time' in hourly:
            times = hourly['time']; temps = hourly.get('temperature_2m', []); codes = hourly.get('weathercode', [])
            for i in range(len(times)):
                dt = datetime.strptime(times[i], "%Y-%m-%dT%H:%M")
                if dt >= now - timedelta(hours=1):
                    forecast_hourly.append({'time': dt.strftime("%H:%M"), 'temp': temps[i] if i < len(temps) else 0, 'code': codes[i] if i < len(codes) else 0})
                    if len(forecast_hourly) >= 24: break
...
def upload_dataset():
    global real_elec_df
    file_path = filedialog.askopenfilename(title="통합 데이터셋 업로드", filetypes=(("CSV files", "*.csv"), ("Excel files", "*.xlsx *.xls"), ("All files", "*.*")))
    if not file_path: return
    try:
        df = pd.read_csv(file_path, encoding='utf-8') if file_path.endswith('.csv') else pd.read_excel(file_path)
    except:
        df = pd.read_csv(file_path, encoding='cp949') if file_path.endswith('.csv') else pd.read_excel(file_path, skiprows=4)
...
def sign_up_action():
    ...
    try:
        res = requests.get(f"{FIREBASE_URL}/{user_id}.json").json()
        if isinstance(res, dict) and "error" in res:
            return messagebox.showerror("권한 오류", "파이어베이스 규칙(Rules)이 잠겨 있습니다.")
        if res is not None:
            return messagebox.showerror("가입 실패", "이미 존재하는 아이디입니다.")
        requests.put(f"{FIREBASE_URL}/{user_id}.json", json={"password": hash_password(user_pw), "is_approved": False})
...
def login_action():
    ...
    user_id, user_pw = entry_id.get().strip(), entry_pw.get().strip()
    try:
        user_info = requests.get(f"{FIREBASE_URL}/{user_id}.json").json()
        if user_info and user_info.get('password') == hash_password(user_pw) and user_info.get('is_approved', True):
...
left_login.pack(side="left", fill="y"); left_login.pack_propagate(False)

tk.Label(left_login, text="DTRO", font=("Arial Black", 54, "bold"), fg="#FFFFFF", bg=DTRO_BLUE).pack(pady=(120, 0))
tk.Label(left_login, text="대구교통공사", font=FONT_TITLE, fg=DTRO_GREEN, bg=DTRO_BLUE).pack()
tk.Label(left_login, text="기상·전력 통합 데이터센터", font=FONT_BODY, fg="#E0E0E0", bg=DTRO_BLUE).pack(pady=(5, 0))
...
tk.Label(right_login, text="관리자 로그인", font=FONT_L, bg="#FFFFFF", fg="#333333").pack(anchor="w", pady=(60, 5))
tk.Label(right_login, text="시스템 접근을 위해 로그인해 주세요.", font=FONT_BODY, bg="#FFFFFF", fg="#888888").pack(anchor="w", pady=(0, 30))

tk.Label(right_login, text="아이디 (사번)", font=FONT_BTN, bg="#FFFFFF", fg="#333333").pack(anchor="w")
...
tk.Label(right_login, text="비밀번호", font=FONT_BTN, bg="#FFFFFF", fg="#333333").pack(anchor="w")
...
tk.Button(right_login, text="로그인", command=login_action, font=FONT_BTN, bg=DTRO_BLUE, fg="white", bd=0, pady=10).pack(fill="x", pady=5)
tk.Button(right_login, text="접근 권한 요청 (회원가입)", command=sign_up_action, font=FONT_BODY, bg="#F0F0F0", fg="#333333", bd=0, pady=8).pack(fill="x")
...
if login_success:
    ...
    main_window = tk.Tk()
    main_window.title(f"대구교통공사 통합 기상/전력 시스템 - 접속자: [{logged_in_user}]")
    main_window.geometry("1400x950") 
    main_window.configure(bg=COLOR_BG)

    chart_type_var = tk.StringVar(main_window, value="temp")

    style = ttk.Style()
    style.theme_use('default')
    style.configure('TNotebook', background=COLOR_BG, borderwidth=0)
    style.configure('TNotebook.Tab', padding=[20, 10], font=FONT_SUBTITLE, background=COLOR_BORDER, foreground=COLOR_TEXT_MAIN)
    style.map('TNotebook.Tab', background=[('selected', COLOR_CARD)], foreground=[('selected', COLOR_PRIMARY)])
    
    style.configure("Treeview", font=FONT_BODY, rowheight=28, borderwidth=0, background=COLOR_CARD)
    style.configure("Treeview.Heading", font=FONT_SUBTITLE, background=COLOR_BG, foreground=COLOR_TEXT_MAIN)
    style.map("Treeview", background=[('selected', "#EBF5FF")], foreground=[('selected', COLOR_PRIMARY)])
    ...
    notebook.add(tab_current, text="🌤 실시간/주간 기상예보")
    ...
    def create_sidebar(parent):
        frame = tk.Frame(parent, bg=COLOR_CARD, width=220, highlightthickness=1, highlightbackground=COLOR_BORDER)
        frame.pack(side="left", fill="y", padx=(10, 5), pady=10)
        tk.Label(frame, text="🏢 대상 개소 선택", font=FONT_TITLE, bg=COLOR_CARD, fg=COLOR_PRIMARY).pack(pady=(20, 10))
    ...
    def update_current_display(event=None):
        ...
        if lat is None: lbl_status_curr.config(text="❌ 지역 검색 실패", fg="#EF4444"); return
        
        w = get_current_hybrid_weather(lat, lon)
        if not w: lbl_status_curr.config(text="❌ 기상청 데이터 오류", fg="#EF4444"); return
        
        lbl_status_curr.config(text=f"✅ {full} 데이터 갱신 완료", fg=COLOR_SECONDARY)
        
        lbl_w_icon.config(text=w.get('status', '☀️'))
        lbl_w_temp.config(text=safe_format(w['temp'], "°"))
        lbl_w_mm.config(text=f"최저 {safe_format(w['min'], '°')} / 최고 {safe_format(w['max'], '°')}")
        lbl_w_hum.config(text=safe_format(w['humidity'], '%', True))
        
        pm10_st, pm10_c = get_pm_status(w['pm10'], "pm10")
        lbl_pm10_val.config(text=safe_format(w['pm10'])); lbl_pm10_status.config(text=pm10_st, fg=pm10_c)
        
        pm25_st, pm25_c = get_pm_status(w['pm25'], "pm25")
        lbl_pm25_val.config(text=safe_format(w['pm25'])); lbl_pm25_status.config(text=pm25_st, fg=pm25_c)

        forecasts_daily, forecasts_hourly = get_forecast_weather(lat, lon)
        
        for item in hourly_tree.get_children(): hourly_tree.delete(item)
        for item in short_term_tree.get_children(): short_term_tree.delete(item)
        for item in mid_term_tree.get_children(): mid_term_tree.delete(item)

        if forecasts_hourly:
            for f in forecasts_hourly:
                hourly_tree.insert("", "end", values=(f['time'], WMO_MAP.get(f['code'], "맑음"), f"{f['temp']}°"))

        if forecasts_daily and len(forecasts_daily) >= 7:
            for i in [1, 2]:
                f = forecasts_daily[i]
                dt = datetime.strptime(f['date'], "%Y-%m-%d")
                label = "내일" if i == 1 else "모레"
                short_term_tree.insert("", "end", values=(f"{label} ({dt.strftime('%m/%d')})", WMO_MAP.get(f['code'], "맑음"), f"{f['min']}°", f"{f['max']}°"))
            for i in range(3, 7):
                f = forecasts_daily[i]
                dt = datetime.strptime(f['date'], "%Y-%m-%d")
                mid_term_tree.insert("", "end", values=(f"{i}일 후 ({dt.strftime('%m/%d')})", WMO_MAP.get(f['code'], "맑음"), f"{f['min']}°", f"{f['max']}°"))

    search_frame_c = tk.Frame(tab_current, bg=COLOR_BG, pady=10); search_frame_c.pack(fill="x", padx=15)
    entry_loc_current = tk.Entry(search_frame_c, font=FONT_SUBTITLE, bd=0, highlightthickness=1, highlightcolor=COLOR_PRIMARY, highlightbackground=COLOR_BORDER)
    entry_loc_current.insert(0, "대구 수창동"); entry_loc_current.pack(side="left", fill="x", expand=True, ipady=6, padx=(0, 10))
    entry_loc_current.bind('<Return>', update_current_display)
    tk.Button(search_frame_c, text="지역 검색", command=update_current_display, font=FONT_BTN, bg=COLOR_PRIMARY, fg="white", bd=0, padx=25, pady=6).pack(side="right")
    
    content_c = tk.Frame(tab_current, bg=COLOR_BG); content_c.pack(fill="both", expand=True, padx=10, pady=5)

    left_c = tk.Frame(content_c, bg=COLOR_BG, width=340)
    left_c.pack_propagate(False); left_c.pack(side="left", fill="y", expand=False, padx=(0, 10))
    lbl_status_curr = tk.Label(left_c, text="검색을 완료해 주세요.", font=FONT_BODY, bg=COLOR_BG, fg=COLOR_TEXT_SUB); lbl_status_curr.pack(pady=(0, 5), anchor="w", padx=5)
    
    card_c = tk.Frame(left_c, bg=COLOR_CARD, highlightthickness=1, highlightbackground=COLOR_BORDER)
    card_c.pack(fill="both", expand=True)
    
    top_weather = tk.Frame(card_c, bg=COLOR_CARD, pady=30); top_weather.pack(fill="x")
    lbl_w_icon = tk.Label(top_weather, text="--", font=(FONT_NUM, 50), bg=COLOR_CARD, fg=COLOR_PRIMARY); lbl_w_icon.pack(side="left", padx=(30, 10))
    lbl_w_temp = tk.Label(top_weather, text="--°", font=FONT_XL, bg=COLOR_CARD, fg=COLOR_TEXT_MAIN); lbl_w_temp.pack(side="left")

    lbl_w_mm = tk.Label(card_c, text="최저 --° / 최고 --°", font=FONT_SUBTITLE, bg=COLOR_CARD, fg=COLOR_TEXT_SUB); lbl_w_mm.pack(pady=(0, 20))
    tk.Frame(card_c, height=1, bg=COLOR_BORDER).pack(fill="x", padx=20)
    
    bot_weather = tk.Frame(card_c, bg=COLOR_CARD, pady=20); bot_weather.pack(fill="both", expand=True, padx=20)
    def add_detail_row(parent, label):
        f = tk.Frame(parent, bg=COLOR_CARD, pady=10); f.pack(fill="x")
        tk.Label(f, text=label, font=FONT_BODY, bg=COLOR_CARD, fg=COLOR_TEXT_SUB).pack(side="left")
        v = tk.Label(f, text="--", font=FONT_SUBTITLE, bg=COLOR_CARD, fg=COLOR_TEXT_MAIN); v.pack(side="right")
        s = tk.Label(f, text="", font=FONT_BODY, bg=COLOR_CARD); s.pack(side="right", padx=10)
        return v, s

    lbl_w_hum, _ = add_detail_row(bot_weather, "💧 습도")
    lbl_pm10_val, lbl_pm10_status = add_detail_row(bot_weather, "😷 미세먼지 (PM10)")
    lbl_pm25_val, lbl_pm25_status = add_detail_row(bot_weather, "🦠 초미세먼지 (PM2.5)")

    right_c = tk.Frame(content_c, bg=COLOR_BG)
    right_c.pack(side="right", fill="both", expand=True)

    def create_forecast_panel(parent, title, color, height):
        f = tk.Frame(parent, bg=COLOR_CARD, highlightthickness=1, highlightbackground=COLOR_BORDER)
        f.pack(fill="both", expand=True, pady=(0, 10))
        tk.Label(f, text=title, font=FONT_TITLE, bg=COLOR_CARD, fg=color).pack(anchor="w", padx=15, pady=(15, 5))
        scroll = tk.Scrollbar(f, orient="vertical")
        tv = ttk.Treeview(f, columns=("c1", "c2", "c3", "c4"), show="headings", yscrollcommand=scroll.set, height=height)
        scroll.config(command=tv.yview); scroll.pack(side="right", fill="y")
        tv.pack(fill="both", expand=True, padx=(15, 0), pady=(0, 15))
        tv.heading("c1", text="날짜/시간"); tv.heading("c2", text="날씨"); tv.heading("c3", text="최저기온"); tv.heading("c4", text="최고기온")
        for c in ("c1", "c2", "c3", "c4"): tv.column(c, anchor="center")
        return tv

    hourly_tree = create_forecast_panel(right_c, "⏰ 시간대별 예보 (24시간)", COLOR_PRIMARY, 5)
    hourly_tree.heading("c3", text="기온"); hourly_tree.heading("c4", text="")
    
    short_term_tree = create_forecast_panel(right_c, "📅 단기 예보 (내일 ~ 모레)", COLOR_COMPARE, 2)
    mid_term_tree = create_forecast_panel(right_c, "🗓️ 중기 예보 (주간)", COLOR_SECONDARY, 4)
    ...
    lbl_selected_station = tk.Label(elec_top_frame, text="👈 왼쪽에서 개소를 선택해주세요", font=FONT_TITLE, bg=COLOR_BG, fg=COLOR_PRIMARY); lbl_selected_station.pack(side="left")

    date_frame = tk.Frame(elec_top_frame, bg=COLOR_BG); date_frame.pack(side="right")
    tk.Button(date_frame, text="엑셀 저장", command=lambda: export_treeview_to_excel(elec_tree, "통합분석"), font=FONT_BTN, bg=COLOR_SECONDARY, fg="white", bd=0, pady=6, padx=15).pack(side="right", padx=(5, 0))
    tk.Button(date_frame, text="데이터 불러오기", command=lambda: load_elec_and_weather_data(), font=FONT_BTN, bg="#F59E0B", fg="white", bd=0, pady=6, padx=15).pack(side="right", padx=(10, 0))
    
    entry_elec_end = tk.Entry(date_frame, font=FONT_BODY, bd=0, highlightthickness=1, highlightbackground=COLOR_BORDER, width=12); entry_elec_end.insert(0, (datetime.now() - timedelta(days=1)).strftime("%Y-%m-%d")); entry_elec_end.pack(side="right", padx=5, ipady=4)
    tk.Label(date_frame, text="~", font=FONT_BODY, bg=COLOR_BG).pack(side="right")
    entry_elec_start = tk.Entry(date_frame, font=FONT_BODY, bd=0, highlightthickness=1, highlightbackground=COLOR_BORDER, width=12); entry_elec_start.insert(0, (datetime.now() - timedelta(days=7)).strftime("%Y-%m-%d")); entry_elec_start.pack(side="right", padx=5, ipady=4)
    tk.Label(date_frame, text="조회기간:", font=FONT_BODY, bg=COLOR_BG).pack(side="right", padx=(10, 0))
    ...
    def create_elec_card(parent, title):
        f = tk.Frame(parent, bg=COLOR_CARD, highlightthickness=1, highlightbackground=COLOR_BORDER, padx=20, pady=20); f.pack(side="left", fill="x", expand=True, padx=5)
        tk.Label(f, text=title, font=FONT_SUBTITLE, bg=COLOR_CARD, fg=COLOR_TEXT_SUB).pack(anchor="w")
        lbl_v = tk.Label(f, text="--", font=FONT_L, bg=COLOR_CARD, fg=COLOR_PRIMARY); lbl_v.pack(anchor="w", pady=5)
        return lbl_v
    ...
    elec_chart_frame = tk.Frame(charts_container, bg=COLOR_CARD, highlightthickness=1, highlightbackground=COLOR_BORDER); elec_chart_frame.pack(side="left", fill="both", expand=True, padx=(0, 5))
    elec_fig.patch.set_facecolor(COLOR_CARD)
    elec_canvas = FigureCanvasTkAgg(elec_fig, master=elec_chart_frame); elec_canvas.get_tk_widget().pack(fill="both", expand=True, padx=10, pady=10)
    ...
    weather_right_container = tk.Frame(charts_container, bg=COLOR_CARD, highlightthickness=1, highlightbackground=COLOR_BORDER); weather_right_container.pack(side="right", fill="both", expand=True, padx=(5, 0))
    w_ctrl_frame = tk.Frame(weather_right_container, bg=COLOR_CARD); w_ctrl_frame.pack(fill="x", pady=10, padx=10)
    ...
    def redraw_weather_chart():
        ...
        if not ctype: ctype = "temp" 

        weather_highlight_points.clear(); weather_fig.clear(); ax = weather_fig.add_subplot(111)
        weather_fig.patch.set_facecolor(COLOR_CARD)

        labels = [d['label'] for d in weather_chart_data]
        if ctype == "temp":
            ax.plot(labels, [d['max'] for d in weather_chart_data], marker='o', color='#EF4444', label='최고기온', linewidth=2)
            ax.plot(labels, [d['min'] for d in weather_chart_data], marker='o', color=COLOR_PRIMARY, label='최저기온', linewidth=2)
            max_vals = [d['max'] for d in weather_chart_data if d['max'] is not None]
            min_vals = [d['min'] for d in weather_chart_data if d['min'] is not None]
            if max_vals and min_vals: ax.set_ylim(min(min_vals)-5, max(max_vals)+5)
        elif ctype == "hum": ax.bar(labels, [d['hum'] for d in weather_chart_data], color=COLOR_SECONDARY, alpha=0.6, label='평균 습도'); ax.set_ylim(0, 110)
        elif ctype == "pm":
            ax.plot(labels, [d['pm10'] for d in weather_chart_data], marker='s', color='#F59E0B', label='PM10', linewidth=2)
            ax.plot(labels, [d['pm25'] for d in weather_chart_data], marker='^', color='#8D6E63', label='PM2.5', linewidth=2)
            
        ax.set_title("기상 변화 트렌드", fontsize=10, fontweight='bold', fontfamily=FONT_FAMILY); ax.legend(loc='upper right', prop={'family': FONT_FAMILY, 'size': 8}); ax.grid(True, linestyle='--', alpha=0.3)
        if len(labels) > 15: ax.set_xticks(range(0, len(labels), max(1, len(labels)//10)))
        ax.tick_params(axis='x', rotation=45, labelsize=8)
        weather_fig.tight_layout()
        weather_annot = ax.annotate("", xy=(0,0), xytext=(10, 15), textcoords="offset points", bbox=dict(boxstyle="round,pad=0.5", fc="#FFFDE7", ec="#FBC02D", lw=1.5, alpha=0.9), fontsize=9, fontfamily=FONT_FAMILY, zorder=10)
        weather_annot.set_visible(False)
        weather_canvas.draw()

    tk.Radiobutton(w_ctrl_frame, text="기온", variable=chart_type_var, value="temp", command=redraw_weather_chart, bg=COLOR_CARD, font=FONT_BODY).pack(side="left", padx=5)
    tk.Radiobutton(w_ctrl_frame, text="습도", variable=chart_type_var, value="hum", command=redraw_weather_chart, bg=COLOR_CARD, font=FONT_BODY).pack(side="left", padx=5)
    tk.Radiobutton(w_ctrl_frame, text="먼지", variable=chart_type_var, value="pm", command=redraw_weather_chart, bg=COLOR_CARD, font=FONT_BODY).pack(side="left", padx=5)
    ...
    elec_tree_frame = tk.Frame(main_elec_frame, bg=COLOR_CARD, highlightthickness=1, highlightbackground=COLOR_BORDER); elec_tree_frame.pack(fill="both", expand=True, padx=5, pady=(0, 5))
    ...
    def load_elec_and_weather_data():
        global elec_chart_data, elec_ax1, elec_ax2, weather_chart_data
        selected = station_tree.selection()
        if not selected: return messagebox.showwarning("선택 오류", "개소를 먼저 선택해주세요.")
        item_text = station_tree.item(selected[0], "text")
        ...
        except ValueError: return messagebox.showerror("오류", "날짜 형식 오류")
        ...
        mult = 100 if item_text == "전체" else 30 if "1호선" in item_text else 40 if "2호선" in item_text else 15
        ...
        for day_str in date_list:
            du = random.uniform(20000, 24000) * mult; dm = random.uniform(1200, 1600) * mult; dc = du * 0.00045
            
            w_info = weather_dict.get(day_str, {})
            w_max_val = w_info.get('max')
            if w_max_val is None:
                w_max_val = random.uniform(22.0, 35.0); w_min_val = random.uniform(15.0, 24.0); w_hum_val = random.uniform(40, 85); w_pm10_val = random.uniform(20, 60); w_pm25_val = random.uniform(10, 35); w_code = random.choice([0, 1, 2, 3])
            else:
                w_min_val = w_info.get('min'); w_hum_val = w_info.get('hum'); w_pm10_val = w_info.get('pm10'); w_pm25_val = w_info.get('pm25'); w_code = w_info.get('code', 0)

            tot_u += du; tot_c += dc
            if dm > max_p: max_p = dm
            d_days.append(day_str); d_usage.append(du); d_max.append(dm)

            consolidated_weather.append({'date': day_str, 'max': w_max_val, 'min': w_min_val, 'hum': w_hum_val, 'pm10': w_pm10_val, 'pm25': w_pm25_val})
            
            parent = elec_tree.insert("", "end", text=f"📁 {day_str}", values=(f"{du:,.1f}", f"{dm:,.1f}", f"{random.uniform(2500,2900)*mult:,.1f}", "10.0", f"{dc:,.2f}", "99.2", "100", WMO_MAP.get(w_code, "--"), safe_format(w_max_val, "°"), safe_format(w_min_val, "°"), safe_format(w_hum_val, "%", True), safe_format(w_pm10_val, "", True), safe_format(w_pm25_val, "", True)))
        ...
        elec_ax1.set_ylabel(y1_label, color=COLOR_PRIMARY, fontweight="bold", fontfamily=FONT_FAMILY)
        elec_ax1.set_ylim(0, max(plot_u) * 1.2 if plot_u else 1); elec_ax1.tick_params(axis='y', labelcolor=COLOR_PRIMARY)
        elec_ax2 = elec_ax1.twinx()
        elec_ax2.plot(plot_days, plot_m, color="#E91E63", marker="o", linewidth=2, label=y2_label)
        elec_ax2.set_ylabel(y2_label, color="#E91E63", fontweight="bold", fontfamily=FONT_FAMILY)
        elec_ax2.set_ylim(0, max(plot_m) * 2.0 if plot_m else 1); elec_ax2.tick_params(axis='y', labelcolor="#E91E63")
        elec_ax1.set_title(f"{title_prefix} {item_text} 전력 트렌드", fontsize=10, fontweight="bold", fontfamily=FONT_FAMILY)
        lines1, labels1 = elec_ax1.get_legend_handles_labels(); lines2, labels2 = elec_ax2.get_legend_handles_labels()
        elec_ax1.legend(lines1 + lines2, labels1 + labels2, loc='upper left', prop={'family': FONT_FAMILY, 'size': 8}); elec_ax1.grid(True, linestyle='--', alpha=0.3)
        ...
        elec_annot = elec_ax1.annotate("", xy=(0,0), xytext=(10, 15), textcoords="offset points", bbox=dict(boxstyle="round,pad=0.5", fc="#FFFDE7", ec="#FBC02D", lw=1.5, alpha=0.9), fontsize=9, fontfamily=FONT_FAMILY, zorder=10)
        ...
        if is_monthly:
            ...
            for w in consolidated_weather:
                weather_chart_data.append({'label': w['date'][5:], 'max': w['max'] if w['max'] is not None else 0, 'min': w['min'] if w['min'] is not None else 0, 'hum': w['hum'] if w['hum'] is not None else 0, 'pm10': w['pm10'] if w['pm10'] is not None else 0, 'pm25': w['pm25'] if w['pm25'] is not None else 0})
    ...
    lbl_comp_station = tk.Label(comp_top_frame, text="👈 왼쪽에서 개소를 선택해주세요", font=FONT_TITLE, bg=COLOR_BG, fg=COLOR_COMPARE); lbl_comp_station.pack(side="left")
    ...
    tk.Button(c_ctrl_frame, text="엑셀 저장", command=lambda: export_treeview_to_excel(c_tv, "연도별비교"), font=FONT_BTN, bg=COLOR_SECONDARY, fg="white", bd=0, pady=6, padx=15).pack(side="right", padx=(5, 0))
    tk.Button(c_ctrl_frame, text="비교 분석", command=lambda: load_compare_data(), font=FONT_BTN, bg=COLOR_COMPARE, fg="white", bd=0, pady=6, padx=15).pack(side="right", padx=(10, 0))
    entry_price = tk.Entry(c_ctrl_frame, font=FONT_BODY, bd=0, highlightthickness=1, highlightbackground=COLOR_BORDER, width=8); entry_price.insert(0, "150"); entry_price.pack(side="right", padx=5, ipady=4)
    tk.Label(c_ctrl_frame, text="평균단가(원/kWh):", font=FONT_BODY, bg=COLOR_BG).pack(side="right", padx=(10, 0))
    entry_y2 = tk.Entry(c_ctrl_frame, font=FONT_BODY, bd=0, highlightthickness=1, highlightbackground=COLOR_BORDER, width=6); entry_y2.insert(0, "2025"); entry_y2.pack(side="right", padx=5, ipady=4)
    tk.Label(c_ctrl_frame, text="비교연도:", font=FONT_BODY, bg=COLOR_BG).pack(side="right", padx=(10, 0))
    entry_y1 = tk.Entry(c_ctrl_frame, font=FONT_BODY, bd=0, highlightthickness=1, highlightbackground=COLOR_BORDER, width=6); entry_y1.insert(0, "2024"); entry_y1.pack(side="right", padx=5, ipady=4)
    tk.Label(c_ctrl_frame, text="기준연도:", font=FONT_BODY, bg=COLOR_BG).pack(side="right")
    ...
    def create_comp_card(parent, title):
        f = tk.Frame(parent, bg=COLOR_CARD, highlightthickness=1, highlightbackground=COLOR_BORDER, padx=20, pady=20); f.pack(side="left", fill="x", expand=True, padx=5)
        tk.Label(f, text=title, font=FONT_SUBTITLE, bg=COLOR_CARD, fg=COLOR_TEXT_SUB).pack(anchor="w")
        lbl_v = tk.Label(f, text="--", font=FONT_L, bg=COLOR_CARD, fg=COLOR_COMPARE); lbl_v.pack(anchor="w", pady=5)
        lbl_s = tk.Label(f, text="--", font=FONT_BODY, bg=COLOR_CARD, fg=COLOR_TEXT_SUB); lbl_s.pack(anchor="w")
        return lbl_v, lbl_s
    ...
    comp_chart_frame = tk.Frame(comp_main_frame, bg=COLOR_CARD, highlightthickness=1, highlightbackground=COLOR_BORDER); comp_chart_frame.pack(fill="both", expand=True, padx=5, pady=5)
    compare_fig.patch.set_facecolor(COLOR_CARD)
    compare_canvas = FigureCanvasTkAgg(compare_fig, master=comp_chart_frame); compare_canvas.get_tk_widget().pack(fill="both", expand=True, padx=10, pady=10)

    # 🎯 트리뷰 (분석 리포트 공간 확보)
    comp_tree_frame = tk.Frame(comp_main_frame, bg=COLOR_CARD, highlightthickness=1, highlightbackground=COLOR_BORDER); comp_tree_frame.pack(fill="x", padx=5, pady=(0, 5))
    ...
    c_tv.heading("month", text="월별"); c_tv.heading("y1_val", text="기준연도 (kWh)"); c_tv.heading("y2_val", text="비교연도 (kWh)"); c_tv.heading("diff", text="증감량 (kWh)"); c_tv.heading("diff_pct", text="증감률 (%)"); c_tv.heading("cost", text="요금 증감 (원)")
    for col in c_cols: c_tv.column(col, width=120, anchor="center")

    # 🎯 AI 증감 요인 분석 리포트 프레임
    analysis_frame = tk.Frame(comp_main_frame, bg=COLOR_CARD, highlightthickness=1, highlightbackground=COLOR_BORDER)
    analysis_frame.pack(fill="x", padx=5, pady=(5, 5))
    tk.Label(analysis_frame, text="💡 AI 증감 요인 분석 리포트", font=FONT_SUBTITLE, bg=COLOR_CARD, fg=COLOR_PRIMARY).pack(anchor="w", padx=15, pady=(10, 0))
    txt_analysis = tk.Text(analysis_frame, height=4, font=FONT_BODY, bg=COLOR_BG, fg=COLOR_TEXT_MAIN, relief="flat", padx=10, pady=10)
    ...
    def load_compare_data():
        ...
        for m in months:
            ...
            y1_data.append(base_usage); y2_data.append(base_usage * random.uniform(0.95, 1.03))

        total_y1 = sum(y1_data); total_y2 = sum(y2_data)
        diff_total = total_y2 - total_y1; diff_pct = (diff_total / total_y1) * 100 if total_y1 else 0
        ...
        color_d = "#EF4444" if diff_total > 0 else COLOR_ACTUAL; sign_d = "증가" if diff_total > 0 else "감소"
        lbl_c_diff.config(text=f"{abs(diff_total):,.0f} kWh {sign_d}", fg=color_d); lbl_c_diff_s.config(text=f"전년 대비 {abs(diff_pct):.2f}% {sign_d}", fg=color_d)
        lbl_c_save.config(text=f"{abs(cost_diff):,.0f} 원", fg=color_d); lbl_c_save_s.config(text=f"전기요금 {'상승' if cost_diff > 0 else '절감'} 추정치", fg=color_d)
        ...
        if diff_total > 0:
            ...
            report += f"▶ 추가 분석: 해당 기간 이용 승객수 증가 및 피크 시간대 공조 설비 가동률 상승이 영향을 미쳤습니다."
        else:
            report += f"▶ 종합: 전년 대비 총 전력량이 {abs(diff_pct):.2f}% 감소(약 {abs(diff_total):,.0f} kWh) 하였습니다.\n"
            report += f"▶ 주요 절감 요인: 고효율 설비 교체 효과 및 {max_month}월의 온화한 기후로 인한 냉난방 부하 감소가 주효했습니다.\n"
            report += f"▶ 추가 분석: 피크 전력 제어 시스템 가동이 실질적인 비용 절감으로 이어졌습니다."

        txt_analysis.config(state="normal"); txt_analysis.delete("1.0", "end"); txt_analysis.insert("1.0", report); txt_analysis.config(state="disabled")

        compare_fig.clear(); ax = compare_fig.add_subplot(111)
        bar_w = 0.35; x1 = [m - bar_w/2 for m in months]; x2 = [m + bar_w/2 for m in months]
        ax.bar(x1, y1_data, width=bar_w, color="#CBD5E1", label=f"{y1_str}년 (기준)")
        ax.bar(x2, y2_data, width=bar_w, color=COLOR_COMPARE, label=f"{y2_str}년 (비교)")
        ax.set_xticks(months); ax.set_xticklabels([f"{m}월" for m in months], fontfamily=FONT_FAMILY)
        ax.set_ylabel("전력 사용량 (kWh)", fontweight="bold", fontfamily=FONT_FAMILY)
        ax.set_title(f"[{item_text}] {y1_str}년 vs {y2_str}년 월별 전력량 비교", fontsize=10, fontweight="bold", fontfamily=FONT_FAMILY)
        ax.legend(prop={'family': FONT_FAMILY}); ax.grid(True, linestyle='--', alpha=0.3, axis='y')
    ...
    lbl_p_title = tk.Label(p_top, text="👈 예측할 개소를 선택하세요", font=FONT_TITLE, bg=COLOR_BG, fg=COLOR_PREDICT)
    ...
    tk.Button(p_ctrl_frame, text="AI 예측/조회 실행", command=lambda: run_prediction(), font=FONT_BTN, bg=COLOR_PREDICT, fg="white", bd=0, pady=6, padx=15).pack(side="right", padx=(10, 0))
    
    cb_model = ttk.Combobox(p_ctrl_frame, values=["Random Forest (Scikit)"], width=20, font=FONT_BODY)
    cb_model.current(0); cb_model.pack(side="right", padx=5)
    
    entry_p_year = tk.Entry(p_ctrl_frame, font=FONT_BODY, bd=0, highlightthickness=1, highlightbackground=COLOR_BORDER, width=8)
    entry_p_year.insert(0, "2026")
    entry_p_year.pack(side="right", padx=5, ipady=4)
    tk.Label(p_ctrl_frame, text="타겟 연도(과거/미래):", font=FONT_BODY, bg=COLOR_BG).pack(side="right")
    
    tk.Button(p_ctrl_frame, text="데이터셋(CSV) 업로드", command=upload_dataset, font=FONT_BTN, bg="#64748B", fg="white", bd=0, pady=6, padx=10).pack(side="right", padx=(0, 10))

    sim_frame = tk.Frame(pred_main_frame, bg=COLOR_CARD, highlightthickness=1, highlightbackground=COLOR_BORDER, padx=15, pady=15)
    sim_frame.pack(fill="x", pady=5)
    tk.Label(sim_frame, text="🔮 연간 변수 시뮬레이터", font=FONT_SUBTITLE, bg=COLOR_CARD, fg=COLOR_PREDICT).pack(side="left", padx=(0, 20))
    tk.Label(sim_frame, text="연간 승객수 증감(%):", font=FONT_BODY, bg=COLOR_CARD).pack(side="left")
    entry_sim_pass = tk.Entry(sim_frame, width=6, font=FONT_BODY, highlightthickness=1, highlightbackground=COLOR_BORDER); entry_sim_pass.insert(0, "5.0"); entry_sim_pass.pack(side="left", padx=(5, 20))
    tk.Label(sim_frame, text="기온 조정치(±°C):", font=FONT_BODY, bg=COLOR_CARD).pack(side="left")
    entry_sim_temp = tk.Entry(sim_frame, width=6, font=FONT_BODY, highlightthickness=1, highlightbackground=COLOR_BORDER); entry_sim_temp.insert(0, "+1.5"); entry_sim_temp.pack(side="left", padx=5)
    ...
    def create_pred_card(parent, title):
        f = tk.Frame(parent, bg=COLOR_CARD, highlightthickness=1, highlightbackground=COLOR_BORDER, padx=20, pady=20); f.pack(side="left", fill="x", expand=True, padx=5)
        lbl_t = tk.Label(f, text=title, font=FONT_SUBTITLE, bg=COLOR_CARD, fg=COLOR_TEXT_SUB); lbl_t.pack(anchor="w")
        lbl_v = tk.Label(f, text="--", font=FONT_L, bg=COLOR_CARD, fg=COLOR_PREDICT); lbl_v.pack(anchor="w", pady=5)
        lbl_s = tk.Label(f, text="--", font=FONT_BODY, bg=COLOR_CARD, fg=COLOR_TEXT_SUB); lbl_s.pack(anchor="w")
    ...
    trend_frame = tk.Frame(p_charts_container, bg=COLOR_CARD, highlightthickness=1, highlightbackground=COLOR_BORDER); trend_frame.pack(side="left", fill="both", expand=True, padx=(0, 5))
    pred_fig.patch.set_facecolor(COLOR_CARD)
    pred_canvas = FigureCanvasTkAgg(pred_fig, master=trend_frame); pred_canvas.get_tk_widget().pack(fill="both", expand=True, padx=10, pady=10)

    feat_frame = tk.Frame(p_charts_container, bg=COLOR_CARD, highlightthickness=1, highlightbackground=COLOR_BORDER); feat_frame.pack(side="right", fill="both", expand=False, padx=(5, 0))
    feat_fig.patch.set_facecolor(COLOR_CARD)
    feat_canvas = FigureCanvasTkAgg(feat_fig, master=feat_frame); feat_canvas.get_tk_widget().pack(fill="both", expand=True, padx=10, pady=10)
    ...
    def run_prediction():
        ...
        c_tot = "#EF4444" if diff_tot > 0 else COLOR_SECONDARY; s_tot = "증가" if diff_tot > 0 else "감소"
        ...
        c_peak = "#EF4444" if diff_peak > 0 else COLOR_SECONDARY; s_peak = "증가" if diff_peak > 0 else "감소"
        ...
        if p_d:
            ax1.plot(p_d, p_u, label=f"전년도 실측치 ({past_year_str})", color="#64748B", marker='o', linewidth=2)
            ax1.set_title(f"[{item_text}] 전년도 월별 전력 사용량", fontsize=10, fontweight="bold", fontfamily=FONT_FAMILY)
            ax1.set_ylabel("사용량 (kWh)", fontweight="bold", fontfamily=FONT_FAMILY)
            ax1.legend(loc='upper right', prop={'family': FONT_FAMILY, 'size': 8}); ax1.grid(True, linestyle='--', alpha=0.3); ax1.tick_params(axis='x', rotation=0, labelsize=8)
        
        if f_dates:
            ax2.plot(f_dates, f_usage, label=plot_label, color=plot_color, marker='o', linestyle=plot_style, linewidth=2)
            ax2.set_title(f"[{item_text}] {target_year_int}년 {plot_title_suffix}", fontsize=10, fontweight="bold", fontfamily=FONT_FAMILY)
            ax2.set_ylabel("사용량 (kWh)", fontweight="bold", fontfamily=FONT_FAMILY)
            ax2.legend(loc='upper right', prop={'family': FONT_FAMILY, 'size': 8}); ax2.grid(True, linestyle='--', alpha=0.3); ax2.tick_params(axis='x', rotation=0, labelsize=8)
        ...
        f_ax.barh(s_feat, s_imp, color="#8B5CF6")
        f_ax.set_title("결정 요인(Feature) 중요도", fontsize=10, fontweight="bold", fontfamily=FONT_FAMILY)
        f_ax.set_xlabel("중요도(%)", fontsize=8, fontfamily=FONT_FAMILY)
        f_ax.set_yticklabels(s_feat, fontfamily=FONT_FAMILY)
        feat_fig.tight_layout(); feat_canvas.draw()

    pred_ax = pred_fig.add_subplot(111); pred_ax.text(0.5, 0.5, "데이터셋 업로드 후 [AI 예측/조회] 클릭", ha='center', va='center', color=COLOR_TEXT_SUB, fontfamily=FONT_FAMILY); pred_ax.set_xticks([]); pred_ax.set_yticks([])
    f_ax = feat_fig.add_subplot(111); f_ax.text(0.5, 0.5, "대기 중...", ha='center', va='center', color=COLOR_TEXT_SUB, fontfamily=FONT_FAMILY); f_ax.set_xticks([]); f_ax.set_yticks([])
